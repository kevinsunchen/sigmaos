...
def ls_instances(vpc):
    ...
    for r in response['Reservations']:
        for i in r['Instances']:
            if 'Tags' in i:
                vms.append((i['InstanceId'], i['PublicDnsName'], i['PrivateIpAddress'], i['Tags'], i['PublicIpAddress']))
            else:
                vms.append((i['InstanceId'], i['PublicDnsName'], i['PrivateIpAddress'], i['PublicIpAddress']))
    if vms == []:
        ...
        for vm in vms:
            if args['privaddr']:
                print("VMInstance", name(vm[3]), ":", vm[0], vm[1], vm[2], vm[4])
            else:
                print("VMInstance", name(vm[3]), ":", vm[0], vm[1], vm[4])

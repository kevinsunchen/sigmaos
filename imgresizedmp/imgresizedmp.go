...
func SubmitTask(fsl *fslib.FsLib, job string, provider sp.Tprovider, fn string) error {
	return SubmitTaskMulti(fsl, job, provider, []string{fn})
}

func SubmitTaskMulti(fsl *fslib.FsLib, job string, provider sp.Tprovider, fns []string) error {
	fn := provider.String() + "_" + rd.String(4)
	t := path.Join(sp.IMG, job, "todo", fn)
...
func ThumbName(fn string) string {
	ext := path.Ext(fn)
	fn1 := strings.TrimSuffix(fn, ext) + "-thumb" + ext
	return fn1
}
...
func (imgd *ImgSrv) runTask(t *task, ch chan Tresult) {
	...
	// TODO XXX add err return for ParseTprovider?
	nameSplit := strings.Split(t.name, "_")
	provider := sp.ParseTprovider(nameSplit[0])
	...
	db.DPrintf(db.IMGD, "prep to spawn task %v %v %v", p.GetPid(), p.GetProvider(), p.Args)
	start := time.Now()
	// Spawn proc.
	err := imgd.Spawn(p)
	if err != nil {
		...
		db.DPrintf(db.IMGD, "spawned task %v %v %v", p.GetPid(), p.GetProvider(), p.Args)

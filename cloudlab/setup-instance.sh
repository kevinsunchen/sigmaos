...
# rm $SECRETS
scp -i $DIR/keys/cloudlab-sigmaos ~/.aws/credentials $SSHCMD:~/.aws/
...
then
  ...
  ssh-agent bash -c 'git clone https://github.com/kevinsunchen/sigmaos.git; (cd sigmaos; go mod download;)'

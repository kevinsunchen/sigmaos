...
import (
	...
	"sigmaos/perf"
	"sigmaos/proc"
	proto "sigmaos/procqprvdrsrv/proto"
	"sigmaos/scheddclnt"
	sp "sigmaos/sigmap"
	"sigmaos/sigmasrv"
)
...
func (pq *ProcQPrvdr) Enqueue(ctx fs.CtxI, req proto.EnqueueRequest, res *proto.EnqueueResponse) error {
...
func (pq *ProcQPrvdr) addProc(p *proc.Proc) chan string {
	pq.mu.Lock()
	defer pq.mu.Unlock()
...
func (pq *ProcQPrvdr) runProc(kernelID string, p *proc.Proc, ch chan string, enqTS time.Time) {
...
func (pq *ProcQPrvdr) GetProc(ctx fs.CtxI, req proto.GetProcRequest, res *proto.GetProcResponse) error {
	db.DPrintf(db.PROCQ, "GetProc request by %v", req.KernelID)
	prvdr := proc.Tprovider(req.GetProviderInt())

	for {
		pq.mu.Lock()
		// Iterate through the realms round-robin.
		for r, q := range pq.qs {
			p, ch, ts, ok := q.Dequeue(prvdr)
			if ok {
...
func (pq *ProcQPrvdr) getRealmQueue(realm sp.Trealm) *QueuePrvdr {
	pq.realmMu.RLock()
...
func (pq *ProcQPrvdr) tryGetRealmQueueL(realm sp.Trealm) (*QueuePrvdr, bool) {
	q, ok := pq.qs[realm]
	return q, ok
}
